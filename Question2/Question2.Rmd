...
# Month Days
london_weather_month_day <- london_weather %>% 
    mutate(date = ymd(date)) %>% 
    mutate(months = format(date, "%B"), days = format(date, '%A')) %>% 
    ...
    summarise(avgCloud = mean(cloud_cover, na.rm = TRUE),
              ...
              avgMin = mean(min_temp, na.rm = TRUE),
              avgPrecipitation = mean(precipitation, na.rm = TRUE)) %>% 
    arrange(match(months, month.name), match(days, weekdays(ISOdate(1, 1, 1:7))))

london_weather_month_day %>% 
    ggplot() + 
    geom_ribbon(aes(months,ymin=avgMin,ymax=avgMax),color="yellow",alpha=0.5)

london_weather_months %>% 
    ggplot() +
    geom_col(aes(x = months, y = avgMin)) +
    geom_histogram(aes(x = months, y = avgMax)) +
